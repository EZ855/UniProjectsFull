...
export const Menu = ({ items }) => {
  ...
  return (
      <div className="menu-container">
        ...
        {open && items.map((item, i) => <MenuItem onClick={() => console.log(item)} title={item} key={i + 1} />)}
      </div>
  )
};
...
export const MenuButton = ({ open, onClick }) => {
  return (
      <button onClick={onClick} className="menu-button" aria-label="Toggle Menu" aria-expanded={open}>
...
export const MenuItem = ({ onClick, title }) => {
  return (
      <a href="#" onClick={() => onClick(title)} className="menu-item">{title}</a>
  );
};
