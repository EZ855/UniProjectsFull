...
def test_one_channel(clear):
    ...
    info = {'token': token}
    resp = requests.get(config.url + 'channels/listall/v2', params = info)
...
def test_all(clear):
    ...
    assert resp.status_code == 200
    resp_data = resp.json()
    assert [{'channel_id': 1, 'name': 'first'}, {'channel_id': 2, 'name': 'second'}, {'channel_id': 3, 'name': 'third'}] == resp_data['channels']
