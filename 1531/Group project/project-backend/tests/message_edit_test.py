...
def test_functionality(clear):
    ...
    edit_info = {'token': token,'message_id': 1, 'message': "Hello World?"}
    requests.put(config.url + 'message/edit/v1', json = edit_info)
...
def test_empty(clear):
    ...
    edit_info = {'token': token,'message_id': 1, 'message': ""}
    requests.put(config.url + 'message/edit/v1', json = edit_info)
...
def test_invalid_message_id(clear):
    ...
    edit_info = {'token': token,'message_id': 1, 'message': "Hello World?"}
    resp = requests.put(config.url + 'message/edit/v1', json = edit_info)
    assert resp.status_code == InputError.code
    
def test_invalid_message_len(clear):
    register_info = {'email':'monty@mail.valid' , 'password': 'password', 'name_first': 'monty', 'name_last': 'python'}
    reg_response = requests.post(config.url + 'auth/register/v2', json = register_info) # registering user
    reg_response_data = reg_response.json()
    token = reg_response_data['token']
    ...
    edit_info = {'token': token,'message_id': 1, 'message':"a" * 1001}
    resp = requests.put(config.url + 'message/edit/v1', json = edit_info)
    assert resp.status_code == InputError.code
...
def test_invalid_access(clear):
    ...
    edit_info = {'token': token2,'message_id': 1, 'message': "Hello World?"}
    resp = requests.put(config.url + 'message/edit/v1', json = edit_info)
