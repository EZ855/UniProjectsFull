...
main:
    ...
    # PUT YOUR CODE HERE
    mul     $t2, $t0, 96                                    # calculates address of start of specified row
    mul     $t3, $t1, 4                                     # calculates bytes for specified column
    add     $t4, $t2, $t3                                   # adds which column onto row address
    la      $t5, array                                      #   
    add     $t6, $t5, $t4                                   # finds &array[x][y] puts into $t6
    
    lw      $a0, ($t6)                                      # finds array[x][y] puts into $a0
    li $v0, 1          # printf("%d", array[x][y])
    syscall
