########################################################################
# COMP1521 21T2 -- Assignment 1 -- Minesweeper Constants!
#
...
reveal_grid_body_loop1:
        ...
        la   $t4, grid             #   
        add  $t4, $t4, $t3         #   finds address of grid[row][col]; $t4
        
        li   $t2, 0
        or   $t3, $t2, IS_RVLD_MASK
        sb   $t3, ($t4)            #   store number in grid[row][col]
...
mark_cell__body:
        ...
        li   $t0, IS_RVLD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, mark_cell_if_false0     # if (grid[row][col] & IS_RVLD_MASK) {
        
mark_cell_if_true0:

        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, mark_cell_if_false1        # if (debug mode) {
...
mark_cell_if_false0:
        li   $t0, IS_MRKD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, mark_cell_if_false2    # if (grid[row][col] & IS_MRKD_MASK) {
...
reveal_cell__body:
        ...
        li   $t0, IS_MRKD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_false0     # if (grid[row][col] & IS_MRKD_MASK) {
        
reveal_cell_if_true0:

        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, reveal_cell_if_false1        # if (debug mode) {
        
reveal_cell_if_true1:
        j       reveal_cell__epilogue       # return }
        
reveal_cell_if_false1:
        la   $a0, reveal_error      # printf("Cannot reveal a marked cell.\n");
        li   $v0, 4
        syscall
        j       reveal_cell__epilogue     # return }
        
reveal_cell_if_false0:
        li   $t0, IS_RVLD_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_false2    # if (grid[row][col] & IS_RVLD_MASK) {
        
reveal_cell_if_true2:
        lw      $t0, debug_mode
        li      $t1, 1
        bne     $t0, $t1, reveal_cell_if_false3        # if (debug mode) {
        
reveal_cell_if_true3:
        j       reveal_cell__epilogue       # return }
        
reveal_cell_if_false3:
        la   $a0, already_revealed      # printf("Cell is already revealed.\n");
        li   $v0, 4
        syscall
        j       reveal_cell__epilogue     # return }

reveal_cell_if_false2:                    
        li   $t0, IS_BOMB_MASK
        and  $t1, $t0, $t5
        bne  $t0, $t1, reveal_cell_if_false4    # if (grid[row][col] & IS_BOMB_MASK) {
        
reveal_cell_if_true4:
        li   $t0, LOSE
        sw   $t0, game_state          # game_state = LOSE
        
reveal_cell_if_false4:      # }
        li   $t0, VALUE_MASK
        and  $t1, $t0, $t5
        move $s0, $t4               # stores &grid[row][col] in $s0
        move $s1, $t5               # stores grid[row][col] in $s1
        bnez $t1, reveal_cell_if_false5    # if (grid[row][col] & VALUE_MASK==0) {

reveal_cell_if_true5:
        jal  clear_surroundings            # clear_surroundings(row, col);
        
        j    reveal_cell_if5_end
        
reveal_cell_if_false5:          #  }
        li   $t0, IS_RVLD_MASK            # loads IS_RVLD_MASK
        or   $s1, $s1, $t0                     # grid[row][col] |= IS_RVLD_MASK
        sb   $s1, ($s0)                   # storing new grid[row][col]
        
        lw   $t0, game_state
        li   $t1, LOSE
        beq  $t0, $t1, reveal_cell_if5_end  #     if (game_state != LOSE) {
        
reveal_cell_if_true6:
        lw   $t0, cells_left
        addi $t0, -1
...
reveal_cell_if5_end:   # }
        ...
        lw   $t0, cells_left
        bnez $t0, reveal_cell_if_false7         #   if (cells_left == 0) {
reveal_cell_if_true7:
        li   $t0, WIN
        sw   $t0, game_state          # game_state = WIN     
        
reveal_cell_if_false7: # }
...
clear_surroundings__body:
        ...
        li      $t0, IS_RVLD_MASK
        and     $t1, $t0, $t5
        bne     $t0, $t1, clear_surroundings_if_false0    # if (grid[row][col] & IS_RVLD_MASK) {
        
clear_surroundings_if_true0: 
        j       clear_surroundings__epilogue    #   return; }
        
clear_surroundings_if_false0:             #   // Reveal the cell.
        li   $t0, IS_RVLD_MASK            # loads IS_RVLD_MASK
        ...
        li   $t0, VALUE_MASK
        and  $t1, $t0, $t5
        beqz $t1, clear_surroundings_if_false1    # if (grid[row][col] & VALUE_MASK) {
        
clear_surroundings_if_true1: 
        j       clear_surroundings__epilogue    #   return; }

clear_surroundings_if_false1: 
...
update_highscore_loop0:
        la      $t1, user_name
        add     $t1, $t1, $t0
        lb      $t2, ($t1)                      # user_name[i];   $t2
        beqz    $t2, update_highscore_loop_end0 # while (user_name[i] != '\0') {
        
        la      $t1, high_score
        addi    $t1, $t1, N_INT_BYTES
        add     $t1, $t1, $t0                   # &high_score.name[i];    $t1
...
print_scores_loop0:
        ...
        la      $t3, scores
        mul     $t4, $t2, $t0
        add     $t4, $t4, $t3                   # calculates &scores[i]   $t4
        
        lw      $t3, ($t4)                      # loads curr.score into $t3
        li      $t5, -1
