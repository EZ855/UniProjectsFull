...
Graph GraphMST(Graph g) {
    ...
    // Making edge list using priority queue
    PQ pq = PQNew();
    int vertices_num = g->nV;
    ...
    // Creating mst graph
    Graph mst_graph = GraphNew(g->nV);
    int edges = 0;
    // Using kruskal's algorithm to create mst
    while (!PQIsEmpty(pq)) {
        // Get smallest edge
        Edge e = PQExtract(pq);
        // Insert edge
        if (GraphInsertEdge(mst_graph, e)) {
            edges += 1;
        }
        
        // If graph now has a cycle, remove the edge
        if (GraphHasCycle(mst_graph)) {
            if (GraphRemoveEdge(mst_graph, e.v, e.w)) {
            edges -= 1;
            }
        }
    }
    
    // If minimum spanning tree doesn't have V - 1 edges, returns NULL  
    if (edges != GraphNumVertices(g) - 1) {
        GraphFree(mst_graph);
        PQFree(pq);
        return NULL;
    }
